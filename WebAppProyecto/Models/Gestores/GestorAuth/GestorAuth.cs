...
using WebAppProyecto.Models.Clases.Seguridad;
using WebAppProyecto.Models.Clases.Usuario;

namespace WebAppProyecto.Models.Gestores.GestorAuth
{
    public class GestorAuth
    {
        public Response GetResponse(string usuario, string password)
        {
            Response Usuarios = new Response();
            int us_Id = 0;
            string us_user = "";
            string us_password = "";
            string us_estado = "";
            ...
            string conexion = ConfigurationManager.ConnectionStrings["Tesis"].ToString();
            using (SqlConnection conn = new SqlConnection(conexion))
            {
                SqlCommand sqlCommand = conn.CreateCommand();
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
                sqlCommand.CommandText = "Usuario_x_User";//buscara dentor de la tabla usuario cuando se cumpla el usuario
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        Usuario usuario1 = new Usuario(idUsuario, usuario_bd, password_bd, estado);
                        lista.Add(usuario1);
                    ...
                    if (us_user.Length != 0 || password.Length != 0)
                    {
                        string contraHash = objSecurity.EncodePassword(password);
                        if (usuario == us_user && contraHash == us_password)
                        {
